...
List<CategoryExpense> getCategoryExpenses(List<ExpenseEntity> expenses) {
  ...
  category2Entities.forEach((expenseCategory, filteredExpenses) {
    categoryExpenses.add(CategoryExpense(
        ...
        amount: getTotalAmount(filteredExpenses),
        expenseEntities: filteredExpenses));
  });
...
Map<DateTime, List<ExpenseEntity>> getMonthlyExpensesMap(
    List<ExpenseEntity> expenses) {
  Map<DateTime, List<ExpenseEntity>> monthlyExpensesMap = HashMap();
  for (final expense in expenses) {
    DateTime curMonth =
        DateTime(expense.createdAt.year, expense.createdAt.month);
  ...
  Map<DateTime, List<ExpenseEntity>> sortedMonthlyExpensesMap = Map.fromEntries(
      monthlyExpensesMap.entries.toList()
